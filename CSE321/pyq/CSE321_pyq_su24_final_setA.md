---
source: CSE321_pyq_su24_final_setA.docx (the original, not included in this repo)
...
Now apply Peterson’s solution to simulate the above operations so that the integrity of the account balance is preserved. Show the account balance after each operation. The constraints and initial conditions of the system are given below:
- Each operation takes 3s to complete.
- Each employee must hand over the system access to the other employee every 9s.
- Shoumo ($i=0, j=1$).
- Dipu ($i=1, j=0$).
- `turn = 0`.
...
| 1. / CO2 | a) Find outputs of the given code: / int main(){ / pid_t a,b; / int x[]={7,4}; / a=fork(); / if(a<0){ / printf("fork failed\n"); / } / else if(a==0){ / x[0]=x[0]*5; / x[1]=x[1]/2; / } / else{ / x[0]=x[0]+5; / x[1]=x[1]-5; / wait(); / x[0]=x[0]+x[1]; / x[1]=x[1]-x[0]; / } / b=fork(); / if(b<0){ / printf("fork failed\n"); / } / else if(b>0){ / wait(); / x[0]=x[0]%2; / x[1]=x[1]%2; / } / else{ / x[0]=x[0]*x[0]; / x[1]=x[1]*x[1]; / } / for(int i=0;i<2;i++){ / printf("x[%d] = %d\n",i,x[i]); / } / return 0; / } | [4] |

|  | b) Potato Bank Limited has two employees, Shoumo and Dipu who are responsible for maintaining the balance of a shared account. The system allows Shoumo and Dipu to either deposit or withdraw money from the account. To ensure the integrity of the account balance, the system needs a synchronization mechanism so that only one employee can modify the account balance at any time. Shoumo and Dipu perform 4 operations each on the bank account having a starting balance of 2500 taka. After performing operations on the account, both the employees get feedback from the customer which is equivalent to 3 operations. The operations Shoumo and Dipu performs are given below: / | Shoumo | Dipu | / | Deposit 1000 | Withdraw 1000 | / | Withdraw 2000 | Withdraw 500 | / | Deposit 5500 | Deposit 3000 | / | Withdraw 1100 | Withdraw 500 | / Now apply Peterson’s solution to simulate the above operations so that the integrity of the account balance is preserved. Show the account balance after each operation. The constraints and initial conditions of the system are given below: / Each operation takes 3s to complete. / Each employee must hand over the system access to the other employee every 9s. / Shoumo (i=0, j=1). / Dipu (i=1, j=0). / turn = 0. / flag[0] = true, flag[1] = true. / Dipu starts working first. / The algorithm for Peterson’s solution is provided below: / | do { / flag[i] = true; / turn = j; / while( flag[j] && turn==j); / //Critical Section / flag[i] = false; / // Remainder section / } while(true); | | [6] |

| 2. CO1 |  |  |

|  | Consider the set of processes above, with the length of the CPU burst times given in milliseconds. Draw the Gantt charts illustrating the execution of these processes using preemptive priority (smaller number implies a higher priority) and Round Robin (time quantum = 8 ms) scheduling. Calculate average waiting time and average turnaround time for the above scheduling algorithms and identify which algorithm is the best with proper justification. | [3+2+3+2+1] |

| 3. / CO3 | a) Given memory partitions of 525k, 655k, 425k, 244k, 305k and 855k (in order, bottom to top). Apply first fit and best fit algorithms to place processes with space requirements of 422k, 615k, 420k, 515k and 395k. A constraint is being maintained which is that only one process can be allocated in a partition and in the main memory lower to higher memory addresses are arranged in top to bottom order. Explain which algorithm makes the most effective use of memory and find whether there is any external fragmentation or not. | [2+2+2] |

|  | b) In a memory system, each frame has a size of 3 KB. Placing a process with 4 pages results in an internal fragmentation of 321 Bytes. Find the size of the process in Bytes. [1 KB = 1024 Bytes] | [2] |

|  | c) Consider a system with TLB miss ratio of 25%, and TLB search time of 15 ns. If Effective Access Time is 161.25 ns, find the memory access time. | [2] |

| 4. CO3 | a) In a system the size of the main memory is 60 Bytes and size of each page is 4 Bytes. In a certain moment CPU generates logical addresses 10, 1, 15, 26 and 7 of a process respectively. Logical memory and page table of the process is given below. / Find corresponding physical addresses of generated logical addresses. If the physical address is not valid then write “invalid”. / Map user’s view of the main memory. | [5+2] |

|  | b) Consider a computer with a main memory that has 3 frames and page reference string of 0-6 pages: [5 4 5 2 5 1 1 6 2 0 4 0]. The page reference string represents the order in which the pages are accessed by a program. Apply FIFO & LRU algorithm to simulate the page replacement that occurs when the main memory can hold at most 3 pages at a time and record the number of page faults. Now compare results and mention which algorithm performs better in this scenario. | [3+3+1] |
